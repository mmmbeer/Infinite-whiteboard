...
function drawEdges(ctx) {
  ...
  state.board.edges.forEach((edge) => { const from = nodeMap.get(edge.from); const to = nodeMap.get(edge.to); if (!from || !to) return; const [sx, sy] = anchorPoint(from, edge.fromAnchor); const [ex, ey] = anchorPoint(to, edge.toAnchor); const horizontal = ["e", "w"].includes(edge.fromAnchor); const distance = Math.max(70, Math.min(240, (horizontal ? Math.abs(ex - sx) : Math.abs(ey - sy)) * .45)); const offsets = { n: [0, -distance], e: [distance, 0], s: [0, distance], w: [-distance, 0] }; const a = offsets[edge.fromAnchor] || [distance, 0]; const b = offsets[edge.toAnchor] || [-distance, 0]; ctx.save(); ctx.strokeStyle = "#cbd2d0"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(sx, sy); if (state.board.settings?.connectionType === "straight") ctx.lineTo(ex, ey); else ctx.bezierCurveTo(sx + a[0], sy + a[1], ex + b[0], ey + b[1], ex, ey); ctx.stroke(); if (edge.label) { ctx.fillStyle = "#fffdfd"; ctx.font = "12px system-ui"; ctx.fillText(edge.label, (sx + ex) / 2, (sy + ey) / 2 - 7); } ctx.restore(); });
}
